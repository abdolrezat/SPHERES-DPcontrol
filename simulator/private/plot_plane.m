function plot_plane(~,x,y,theta2q2a,sep_t,n_sampling,a,sep,col_g,col_gh,head_LineWidth,body_LineWidth,tick_spacing_n)
% sampling points:
% method 1:
...
x0 = x(1);
y0 = y(1);
theta2q2a_0 = theta2q2a(1);
%p0 = x(1)^2 + y(1)^2;
for counter = 1:length(x)
    if(   abs(x(counter) - x0)  > sep   || ...
            abs(y(counter) - y0)  > sep || ...
            (    (abs(theta2q2a(counter) - theta2q2a_0 ) > sep_t)   && (  (abs(x(counter) - x0)  > sep/3) || (abs(y(counter) - y0)  > sep/3) ) ) )
        %                  if(x(counter)^2 + y(counter)^2 - p0 > sep)
        i_sampling(end+1) = counter;
        x0 = x(counter);
        y0 = y(counter);
        theta2q2a_0 = theta2q2a(counter);
...
for ii= 1:length(i_sampling)
    ...
    plot(Px([2,3,4,1]),Py([2,3,4,1]),'Color', [col_g col_g col_g], 'LineWidth', body_LineWidth)
