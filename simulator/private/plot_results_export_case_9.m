function plot_results_export_case_9(obj)
...
left_color = [.5 .5 0.5];
right_color = [0.3 .3 .3];
figure('color','white','defaultAxesColorOrder',[left_color; right_color])
plot(T_ode45(1:end-1), obj.history.W_hist(1:end-1,2),'--','Color', [col_3_angle col_3_angle col_3_angle],'LineWidth', LineWidth_3_angle)
set(gca, 'TickDir','out','Box','off','GridLineStyle', gridLineStyle, 'GridAlpha', gridAlpha)
ylim([0.8 1.2])
hold on
grid on
plot(T_ode45(1:end-1), T_ode45(1:end-1)*0 + 1,'-','Color', [0.4 0.4 0.4],'LineWidth', 0.4)
yticks([0.8 0.9 1 1.1 1.2])
ylabel('W_{mf}')
yyaxis right

yticks([0.05 0.5 0.95])
yticklabels({'Position Only','Equal Weights','Attitude Only'})
