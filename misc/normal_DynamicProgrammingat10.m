function S = normal_DynamicProgrammingat10(varargin)

%% generate controller
% if nargin == 0
    controller.name = 'controller_normal_DynamicProgramming'; %name of controller, will be saved under /controller directory
...
simulator_opts.h = 0.01; %simulation fixed time steps
  simulator_opts.T_final = 150; %simulation Tfinal
    ...
    q0 = flip(angle2quat(deg2rad(120),deg2rad(190),deg2rad(200))); %initial angles offset (yaw,pitch,roll)
...
%% generate controller
    ...
    controller.Tm = 200; % Tfinal for Moment controller run
    %time variables
