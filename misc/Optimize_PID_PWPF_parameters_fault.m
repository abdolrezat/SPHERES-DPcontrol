function [x,f]  = Optimize_PID_PWPF_parameters_fault( )
%OPTIMIZE_PID_PARAMETERS Optimize Kp,Ki,Kd 
%   optimizes parameters to get minimum of (1/2)*x'*Q*x + (1/2)*v'*Q*v 
% P0 = [0.1000    0.1062    0.7641    0.9640    0.8761];
%best search P = [0.6880    0.1020    0.9725    0.6705    0.9735] 
% K =[29.2825  181.9300]
P0 = [0.6880    0.1020    0.9725    0.6705    0.9735];
% KF0 = [29.2825  181.9300];
lb =[0 0 0.2 0 0];
ub = [50 1 1 1 1];
...
[x,f] = patternsearch(@objective_function,[P0,KF0],[],[],[],[],lb,ub,[],options);
...
KF = x(6:7);
R0 = [-0.5 -0.5 -0.5]*2*30;
total_cost = run_PID_control(R0, P, KF);
...
function trip_cost = run_PID_control(POS_i, P, KF)
