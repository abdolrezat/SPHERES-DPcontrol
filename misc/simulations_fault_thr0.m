function simulations_fault_thr0()
% Parameters of the representative maneuver
dr0 = [-15 0 0]; %initial relative position offset
...
%% Dynamic Programming controllers
    % control allocations
        %no control allocation
        sim8 = simulation_fault_DP_noallocation(simulator_opts);
        sim8.plot_optimal_path
        % 6 channels PWPF Modulators tuned for fault
        sim1 = simulation_fault_DP_PWPF(simulator_opts);
        fuel_consumption(sim1)
        % 6 channels Quadratic Programming
        sim2 = simulation_fault_DP_QuadProg(simulator_opts);
        sim2.plot_optimal_path
        fuel_consumption(sim2)
        
        % 2xPWPF tuned for fault + 4xPWPF tuned for normal channels
        sim5 = simulation_fault_DP_PWPF_2f4n(simulator_opts);
        fuel_consumption(sim5)
        
        % 2xPWPF tuned fault + 4 channels Quadratic Programming
        sim6 = simulation_fault_DP_Hybrid_2P4Q(simulator_opts);
        fuel_consumption(sim6)

%% PD control
    %control allocations
        % 6 channels PWPF Modulators tuned for fault
        sim3 = simulation_fault_PID_PWPF(simulator_opts);
        fuel_consumption(sim3)

        % 6 channels Quadratic Programming
        sim4 = simulation_fault_PID_QuadProg(simulator_opts);
...
function SC = simulation_fault_DP_PWPF(simulator_opts)
controller.type = 'DP';
...
PWPFparams = [26.0896    0.8597    0.2015    0.1493    0.8138]; %from optimization
...
function SC = simulation_fault_DP_QuadProg(simulator_opts)
controller.type = 'DP';
...
function SC = simulation_fault_PID_PWPF(simulator_opts)
...
function SC = simulation_fault_PID_QuadProg(simulator_opts)
...
function SC = simulation_fault_DP_PWPF_2f4n(simulator_opts)
...
function SC = simulation_fault_DP_Hybrid_2P4Q(simulator_opts)

Thruster_max_F = 0.12; % (N)
Thruster_dist = (9.65E-2); % (meters)
%simulator variables
simulator_opts.mode = 'fault';   % 'normal' for all thrusters operative or 'fault' for one or more thrusters inoperative
simulator_opts.thruster_allocation_mode = 'Hybrid_2P-x'; % {'active set discrete', 'PWPF', 'Schmitt', 'none'}
simulator_opts.controller_InterpmodeF = 'nearest'; %interpolation mehod of F controller output
simulator_opts.controller_InterpmodeM = 'nearest'; %interpolation mehod of M controller output
simulator_opts.Thruster_max_F = Thruster_max_F;
simulator_opts.Thruster_dist = Thruster_dist;
